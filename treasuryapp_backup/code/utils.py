...
def fetch_account_details(entity_accounts):
    """
    This function helps to create DS based on account number entities
    and their currencies
    """
    acccount_details = {}
    for account in entity_accounts:
        acccount_details[account["Account Number"]] = {
            "Entity": account["Entity"], "Currency": account["Currency"]}
    return acccount_details


def fetch_FX_rates(fx_rates_data):
    """
    This function helps to convert the currenct to trie data structure

    Ex:
        {"USD":{"INR":70,"GBP":0.71},"INR":{"USD":0.014}}
    """
    fx_rates = {}
    for rate_map in fx_rates_data:
        currency = rate_map["FROM"]
        if currency in fx_rates:
            fx_rates[currency].update({rate_map["TO"]: rate_map["RATE"]})
        else:
            fx_rates[currency] = {rate_map["TO"]: rate_map["RATE"]}
    return fx_rates


def fetch_HO_curreny(entities):
    """
    This function helps to fetch the head office currency
    """
...
def fetch_entity_details(entities):
    """
    Converting entities and its currencies to key values pairs

    ex: {"Entity": {"Currency":CURRENCY_CODE}}
    """
    entity_details = {}
    for entity in entities:
        entity_details[entity["Entity Name"]] = {
